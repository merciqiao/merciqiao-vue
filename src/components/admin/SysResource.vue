    ...
    <el-dialog title="新增" :visible.sync="addFormVisible" :close-on-click-modal="false">
      <el-form :model="addForm" label-width="80px" :rules="formRules" ref="addForm">
        <el-form-item label="名称" prop="resoureName">
          <el-input v-model="addForm.resoureName" auto-complete="off" placeholder="资源名称"></el-input>
        </el-form-item>
        <el-form-item label="资源类型" prop="resoureType">
          <el-select v-model="addForm.resoureType" placeholder="请选择">
            <el-option
              v-for="item in resoureTypeOptions"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            ></el-option>
...
export default {
  data() {
    return {
      ...
      permittext: "权限标示", //权限提示文本
      permitplaceholder: "",//权限水印
      resoureTypeTip:'',//资源类型提示
    };
  },
  computed: {
    getPermitTip() {
      var tip = "";
      if (this.addForm.resoureType == "url") {
        this.permittext = "路由path";
        this.permitplaceholder = "路由路径";
        this.resoureTypeTip="页面菜单";
        tip = "路由path为'/index'则配index";
      } else if (this.addForm.resoureType == "button") {
        this.permittext = "按钮权限";
        this.permitplaceholder = "权限标示";
        tip = "配置为后端接口的permission，例如留言添加权限则为liuyan_add";
        this.resoureTypeTip="页面按钮";
      } else  if(this.addForm.resoureType == "module"){
        this.permittext = "权限标示";
        tip = "可为空";
